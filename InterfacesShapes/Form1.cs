...
namespace InterfacesShapes
{
    public partial class Form1 : Form
    {
        ...
        {
            pDiger.Location = new Point(12, 39);
            pHesap.Location = new Point(12, 151);
        ...
        private void bHesapla_Click(object sender, EventArgs e)
        {
            ...
            if (rbDaire.Checked)
            {
                ...
                { MessageBox.Show("Yarıçap boş girilemez!"); return; }

                if (!GirisKontrolEt2(tbYariCap.Text)) 
                { MessageBox.Show("Yarıçap sayısal olmalıdır!"); return; }
                ...
                sonuc = ddlIslem.SelectedIndex == 0 ? 
                    roundedShape.CalculateArea() : roundedShape.CalculateCircumference(); // (Ternary Operator)
            }
            else if (rbDortgen.Checked) 
            {
                if (string.IsNullOrWhiteSpace(tbGenislik.Text) || string.IsNullOrWhiteSpace(tbYukseklik.Text)) 
                { MessageBox.Show("Genişlik ve Yükseklik boş girilemez!"); return; }

                if (!GirisKontrolEt2(tbGenislik.Text) || !GirisKontrolEt2(tbYukseklik.Text)) 
                { MessageBox.Show("Genişlik ve Yükseklik sayısal olmalıdır!"); return; }
                ...
                sonuc = ddlIslem.SelectedIndex == 0 ?
                    shapeWithAngles.CalculateArea() : shapeWithAngles.CalculateCircumference(); // (Ternary Operator)
            }
            else 
            {
                if (string.IsNullOrWhiteSpace(tbGenislik.Text)) 
                { MessageBox.Show("Genişlik boş girilemez!"); return; }
                if (string.IsNullOrWhiteSpace(tbYukseklik.Text)) 
                { MessageBox.Show("Yükseklik boş girilemez!"); return; }

                if (!GirisKontrolEt2(tbGenislik.Text)) 
                { MessageBox.Show("Genişlik sayısal olmalıdır!"); return; }
                if (!GirisKontrolEt2(tbYukseklik.Text)) 
                { MessageBox.Show("Yükseklik sayısal olmalıdır!"); return; }
                ...
                sonuc = ddlIslem.SelectedIndex == 0 ?
                    shapeWithAngles.CalculateArea() : shapeWithAngles.CalculateCircumference(); 
            }
        ...
        bool GirisKontrolEt1(string giris) 
        {
            ...
            foreach(var karakter in giris) 
            {
                ...
                {
                    if (karakter.ToString() == izinVerilen) { bulundu = true; break; }
                }
                if (!bulundu) break;
            }
            return bulundu;
